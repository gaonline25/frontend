...
export async function fetchOurFoundersData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/our-founders-component?depth=2&limit=100`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch our founders data");
    }
    ...
    console.error("Error fetching our founders data:", error);
    return null;
  }
}
