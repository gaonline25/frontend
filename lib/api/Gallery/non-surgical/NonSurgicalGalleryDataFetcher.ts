...
export async function fetchNonSurgicalGalleryData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/non-surgical-gallery-component?depth=2&limit=100`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch non-surgical gallery data");
    }
    ...
    console.error("Error fetching non-surgical gallery data:", error);
    return null;
  }
}
