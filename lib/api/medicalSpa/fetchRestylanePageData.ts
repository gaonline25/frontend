...
export async function fetchRestylanePageData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/restylane-page-component?where[isActive][equals]=true&depth=2&limit=100&draft=false&trash=false`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch Restylane page data: ${res.statusText}`);
    }
    const data = await res.json();
    const activeComponent =
      data.docs?.find((component: any) => component.isActive) || null;
    console.log("Fetched Restylane data:", activeComponent); // Debug log
