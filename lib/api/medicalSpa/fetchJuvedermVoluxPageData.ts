...
export async function fetchJuvedermVoluxPageData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/juvederm-volux-page-component?depth=2&limit=100&draft=false&trash=false`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to fetch Juvederm Volux page data: ${res.statusText}`
      );
    }
