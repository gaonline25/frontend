...
export async function fetchMedicalSpaData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/medical-spa-component?depth=2&limit=100`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch medical spa data");
    }
    ...
    console.error("Error fetching medical spa data:", error);
    return null;
  }
}
