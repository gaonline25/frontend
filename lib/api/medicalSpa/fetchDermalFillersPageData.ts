...
export async function fetchDermalFillersPageData(id = "68ea72218253bd170bd9da6c") {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/dermal-fillers-page-component/${id}?depth=2&draft=false&trash=false`,
      {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to fetch Dermal Fillers page data: ${res.status} ${res.statusText}`
      );
    }
