...
// //       `${PAYLOAD_API_URL}/botox-page-component?depth=2&limit=100`,
// //       {
// //         next: { revalidate: 60 },
// //         cache: "force-cache",
// //       }
...
// //     if (!res.ok) {
// //       throw new Error("Failed to fetch BOTOX page data");
// //     }
...
//   try {
//     const res = await fetch(
//       `${PAYLOAD_API_URL}/botox-page-component/${id}?depth=2&draft=false&trash=false`,
//       {
//         next: { revalidate: 60 },
//         cache: "force-cache",
//       }
//     );
//     if (!res.ok) {
//       throw new Error(`Failed to fetch botox page data: ${res.statusText}`);
//     }
//     const data = await res.json();
...
export async function fetchBotoxPageData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/botox-page-component?where[isActive][equals]=true&depth=2&limit=100&draft=false&trash=false`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
