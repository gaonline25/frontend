...
export async function fetchMedicalSpaInjectablesData() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/medical-spa-injectables-component?depth=2&limit=100`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch medical spa injectables data");
    }
    ...
    console.error("Error fetching medical spa injectables data:", error);
    return null;
  }
}
