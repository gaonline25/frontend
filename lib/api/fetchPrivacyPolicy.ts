...
export async function fetchPrivacyPolicyComponent() {
  try {
    const res = await fetch(
      `${PAYLOAD_API_URL}/privacy-policy-page-component?where[isActive][equals]=true&depth=2&limit=100&draft=false&trash=false`,
      {
        next: { revalidate: 60 },
        cache: "force-cache",
      }
