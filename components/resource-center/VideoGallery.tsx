...
// components/resource-center/VideoGallery.tsx
import React from "react";
import {
  getResponsiveImageUrls,
  getImageUrl,
...
const VideoGallery: React.FC<VideoGalleryProps> = ({ data }) => {
  if (!data) return <div>Unable to load content.</div>;
    ...
    <main id="main">
      ...
      <div className="mod_hero banner mobile-stack" data-s3-module="">
        <div className="wrapper">
          ...
          <picture className="background">
            <img
              src={`${heroBgImages.large}?auto=format,compress&w=1994`}
              loading="eager"
              width="1994"
              height="1206"
              alt={heroBgImages.alt || "Background Image"}
              draggable="false"
            />
          </picture>
          ...
          <div className="row">
            <div className="inner">
              ...
              {heroSection.breadcrumbs && (
                <ul className="partial_breadcrumb" data-s3-partial="">
                  {heroSection.breadcrumbs.map((b: any, i: number) => (
                    <li key={i}>
                      <a
                        href={b.url}
                        role="link"
            ...
            <div className="media">
              <picture className="background">
                <img
                  ...
                  width="1920"
                  height="1440"
                  alt={heroMediaImages.alt || "Hero Image"}
                  draggable="false"
                  loading="lazy"
                />
              </picture>
      ...
      >
        <div className="row">
          <div className="video-grid-container">
            {videoGridSection.videos?.length > 0 ? (
              videoGridSection.videos.map((video: any, index: number) => {
                const thumbnailUrl = video.thumbnail?.customThumbnail
                  ? getImageUrl(video.thumbnail.customThumbnail)
                  : getYouTubeThumbnail(video.youtubeUrl);
                ...
                return (
                  <div className="video" key={index}>
                    ...
                    >
                      <picture>
                        <img
                          src={`${thumbnailUrl}?auto=format,compress&w=800`}
                          loading="lazy"
