...
interface PlasticSurgeryProps {
  data: PlasticSurgeryData | null;
}

const PlasticSurgery: React.FC<PlasticSurgeryProps> = ({ data: pageData }) => {
  useEffect(() => {
    ...
    >
      ...
      {pageIndexListSection?.show && (
        ...
        >
          <div className="row">
            <div className="inner">
              {pageIndexListSection.links?.map((link, index) => (
                <a
                  key={index}
                  href={link.url}
                  className="item"
      ...
      {pageIndexSection?.show && (
        <div className="mod_index container" data-s3-module>
          <div className="row subcontent">
            <div className="inner">
              {pageIndexSection.items?.map((item, index) => (
                <div key={index} className="item">
                  <a
                    className="media-btn"
                    href={item.url}
              ...
              {boxGridSection.boxes?.map((box, index) => (
                <div key={index} className="box wow fadeInUp">
                  ...
                  {box.bulletPoints && box.bulletPoints.length > 0 && (
                    ...
                    >
                      {box.bulletPoints.map((point, idx) => (
                        <li key={idx}>{point.text}</li>
                      ))}
                    </ul>
                  )}
