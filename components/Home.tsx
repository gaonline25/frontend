...
// //   if (media.url) return `${process.env.NEXT_PUBLIC_PAYLOAD_URL || 'https://backend-orpin-six-56.vercel.app'}${media.url}`;
// //   return '';
// // };
...
// // //   if (media.url) {
// // //     const PAYLOAD_URL = process.env.NEXT_PUBLIC_PAYLOAD_URL || "https://backend-orpin-six-56.vercel.app";
...
function SectionRenderer({ section }: { section: Section }) {
  ...
  switch (sectionType) {
    ...
    case "founders_section":
      return (
        <FoundersSectionComponent section={section} style={sectionStyle} />
      );
    case "quote_banner":
      return <QuoteBannerSection section={section} style={sectionStyle} />;
    case "procedures_banner":
...
function AnchorLinksSection({
  ...
  return (
    <div
      className={`container mod_anchor_link ${section.customClasses || ""}`}
      style={style}
      data-s3-module=""
    >
      <div className="row">
...
function TwoColumnSection({
  ...
  return (
    ...
    >
      <div
        className={`row pt-20 ${data.mediaPosition || "media-left"}`}
      ...
      >
        <div className="item">
          <h2>
            ...
            {data.mainHeadingLine1 && (
              <span className="first-block">{data.mainHeadingLine1} </span>
            )}
            {data.mainHeadingLine2 && (
              <span className="second-block">{data.mainHeadingLine2}</span>
            )}
          </h2>
          {data.content && <p>{data.content}</p>}
        </div>
...
// Three Column Stats Section
function ThreeColumnStatsSection({
  section,
  style,
}: {
  section: Section;
              ...
              <h3>
                {col.titleSmall && (
                  <span className="small">
                    {col.titleSmall}
                    {col.titleLine1 && (
                      <span className="block-desktop">{col.titleLine1}</span>
                    )}
                  </span>
                )}
              </h3>
...
function HalfImageSection({
  ...
  return (
    ...
    >
      <div
        className={`row half-image-row no-watermark ${data.flipped ? "flipped" : ""}`}
      >
        <div className="media">
          {data.image && (
            <picture className="background">
      ...
      <div className="wrapper">
        <div className="tabs">
          {data.tabs.map((tab: any, idx: number) => (
            <div
              key={idx}
              className={`tab ${idx === 0 ? "active" : ""} tab${idx + 1}`}
            >
              <div className="row">
                <div className="inner">
                  <h2>
                  ...
                  {tab.buttonText && tab.buttonUrl && (
                    <a
                      href={tab.buttonUrl}
                      className={tab.buttonClasses || "btn white"}
                    >
                      {tab.buttonText}
                    </a>
                  )}
...
function FoundersSectionComponent({
  ...
  return (
    ...
    >
      <div
        className={`row half-image-row ${data.flipped ? "flipped" : ""} ${data.hasWatermark ? "" : "no-watermark"} mb-40 mt-40`}
      >
        <div className="media">
          {data.image && (
            <picture className="background">
...
// Procedures Banner Section
function ProceduresBannerSection({
  section,
  style,
}: {
  section: Section;
            ...
            {data.leftBanner.image && (
              <picture className="background">
                <img
                  ...
                  height={data.leftBanner.imageHeight || 1852}
                  alt={
                    data.leftBanner.imageAlt ||
                    "fit woman in black exercise clothes"
                  }
                  draggable="false"
                />
              </picture>
