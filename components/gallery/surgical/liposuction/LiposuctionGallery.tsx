...
// interface TummyTuckGalleryPageData {
...
// const LiposuctionGallery: React.FC = () => {
//   const [pageData, setPageData] = useState<TummyTuckGalleryPageData | null>(
//     null
//   );
...
//           }/LiposuctionGallery?where[isActive][equals]=true&depth=2&limit=100&draft=false&trash=false`,
//           {
//             next: { revalidate: 60 },
//             cache: "force-cache",
//           }
...
//         if (!res.ok) {
//           throw new Error(
//             `Failed to fetch Tummy Tuck Gallery data: ${res.statusText}`
//           );
//         }
//         const data = await res.json();
//         const activeComponent =
//           data.docs?.find((component: any) => component.isActive) || null;
//         console.log("Fetched Tummy Tuck Gallery data:", activeComponent);
...
//                 item.beforeImage.imageUrl ||
//                 getResponsiveImageUrls({ desktop: item.beforeImage.image })
//                   .desktop;
//               const afterImageUrl =
//                 item.afterImage.imageUrl ||
//                 getResponsiveImageUrls({ desktop: item.afterImage.image })
//                   .desktop;
...
//                   onClick={(e) => {
//                     e.preventDefault();
//                     handleImageClick(index * 2); // Before image index
//                   }}
//                 >
//                   {/* Before Image */}
//                   <picture className="">
//                     <source
//                       srcSet={`${beforeImageUrl}?auto=format,compress&w=${
//                         item.beforeImage.width || 400
//                       }`}
//                       data-lazyload-srcset={`${beforeImageUrl}?auto=format,compress&w=${
//                         item.beforeImage.width || 400
//                       }`}
//                     />
//                     <img
//                       src={`${beforeImageUrl}?auto=format,compress&w=${
//                         item.beforeImage.width || 400
//                       }`}
//                       width={item.beforeImage.width || 400}
//                       height={item.beforeImage.height || 534}
...
//                   <picture className="">
//                     <source
//                       srcSet={`${afterImageUrl}?auto=format,compress&w=${
//                         item.afterImage.width || 400
//                       }`}
//                       data-lazyload-srcset={`${afterImageUrl}?auto=format,compress&w=${
//                         item.afterImage.width || 400
//                       }`}
//                     />
//                     <img
//                       src={`${afterImageUrl}?auto=format,compress&w=${
//                         item.afterImage.width || 400
//                       }`}
//                       width={item.afterImage.width || 400}
//                       height={item.afterImage.height || 466}
...
//   // Flatten before/after images for carousel
//   const carouselImages = gallerySection.galleryItems.flatMap((item) => [
//     {
//       src:
//         item.beforeImage.imageUrl ||
//         getResponsiveImageUrls({ desktop: item.beforeImage.image }).desktop,
//       alt: item.beforeImage.altText,
//     },
//     {
//       src:
//         item.afterImage.imageUrl ||
//         getResponsiveImageUrls({ desktop: item.afterImage.image }).desktop,
//       alt: item.afterImage.altText,
//     },
//   ]);
...
interface LiposuctionGalleryProps {
  data: TummyTuckGalleryPageData | null;
}

const LiposuctionGallery: React.FC<LiposuctionGalleryProps> = ({ data }) => {
  const [isCarouselOpen, setIsCarouselOpen] = useState(false);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
