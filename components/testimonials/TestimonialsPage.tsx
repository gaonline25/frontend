...
import React from "react";
import { getResponsiveImageUrls } from "@/lib/api/fetchTestimonialsData";
import { Star } from "lucide-react";
...
const TestimonialsPage: React.FC<TestimonialsPageProps> = ({
  ...
  if (!testimonialsData) {
    return <div>Unable to load testimonials.</div>;
  }
  ...
  return (
    ...
    >
      ...
      >
        <div className="wrapper">
          <picture className="background">
            {backgroundImage.large && (
              <img
                src={`${backgroundImage.large}?auto=format,compress&w=1994`}
                alt={backgroundImage.alt || "Background Texture"}
                width="1994"
                height="1206"
                loading="eager"
          ...
          <div className="media">
            <picture className="background">
              {mediaImage.large && (
                <img
                  src={`${mediaImage.large}?auto=format,compress&w=1920`}
                  width="1920"
                  height="1206"
                  alt={mediaImage.alt || "Hero image"}
                  loading="lazy"
      ...
      {/* Rating Box Sections */}
      {ratingBoxSections?.map((section: any, i: number) => (
        <div
          key={i}
          className="container mod_rating_box"
          data-s3-module=""
        ...
        >
          <div className="row">
            ...
            <div
              ...
              data-items={section.itemsPerRow || 3}
            >
              {section.locations?.map((location: any, j: number) => {
                const displayName =
                  location.displayName ||
                  `${location.platform} - ${location.name}`;
                return (
                  <div className="item" key={j}>
                    <a
                      href={location.url}
                      target={location.openInNewTab ? "_blank" : "_self"}
                      rel={location.openInNewTab ? "noopener noreferrer" : ""}
                      className="review-box"
                      aria-label={
                        location.ariaLabel ||
                        `${displayName} reviews that open in a new tab`
                      ...
                      <div className="reviews-holder block">
                        ...
                        >
                          {[...Array(5)].map((_, starIndex) => (
                            ...
                            //   style={{
                            //     fill:
                            //       starIndex < location.rating ? "#FFD700" : "#ccc",
                            //   }}
                            // />
                            <i key={starIndex} className="f-icon icon-star"></i>
                          ))}
                ...
                >
                  ...
                  >
                    {[...Array(review.rating || 5)].map((_, starIndex) => (
                      ...
                      //   style={{
                      //     fill:
                      //       starIndex < review.rating ? "#FFD700" : "#ccc",
                      //   }}
                      // />
                      <i key={starIndex} className="f-icon icon-star"></i>
                    ))}
